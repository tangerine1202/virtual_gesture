...
class OneEuroFilter():
  ...
  def save(self):
    self.saver.z = np.concatenate([self.saver.z, self._s_z[np.newaxis, ...]], axis=0)
    self.saver.x = np.concatenate([self.saver.x, self._x_prev[np.newaxis, ...]], axis=0)
    self.saver.dx = np.concatenate([self.saver.dx, self._dx_prev[np.newaxis, ...]], axis=0)
